...
func TestChildRestartTracker_BudgetThenWindowReset(t *testing.T) {
	...
	// exhausted) — this is what triggers fatal escalation in supervisePTYHost.
...
// crashChildScript is a minimal Node "pty-host" used to simulate a supervised
...
// process that recovers after N crashes. It never needs to emit valid
// pty_output NDJSON frames for these tests since the ws side stays idle.
...
// TestRun_PTYHostCrash_RestartsInsteadOfExiting proves Fix 2: a mid-session
// pty-host crash must NOT make Run return (which previously propagated to
// main.go's log.Fatalf and permanently killed the daemon, dropping the user's
// terminal room entirely) — it should be restarted in place while the relay
// session keeps running.
func TestRun_PTYHostCrash_RestartsInsteadOfExiting(t *testing.T) {
	requireNode(t)
...
// TestRun_PTYHostCrashLoop_EscalatesToFatal proves the other half of Fix 2:
...
func TestRun_PTYHostCrashLoop_EscalatesToFatal(t *testing.T) {
	requireNode(t)
	useFastChildTimings(t)
