...
export class RelayControlClient {
  ...
  private handleProofMessage(message: Record<string, unknown>): void {
    // Pie Relay's self-hosted cell can be configured behind an already
    // authenticated bearer-token edge. In that mode the outer WebSocket is
    // authenticated by the HTTP upgrade and the optional cryptographic host
    // challenge is omitted; accept the normal hello acknowledgement directly.
    const directAck = RelayHostHelloAckMessageSchema.safeParse(message)
    if (directAck.success) {
      this.state = 'active'
      this.connectResolve?.(directAck.data)
      this.clearConnectPromise()
      return
    }
    if (message.type === 'host-hello-ack' && typeof message.generation === 'number') {
      this.state = 'active'
      this.connectResolve?.(message as never)
      this.clearConnectPromise()
      return
    }
