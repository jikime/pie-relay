...
function isAllowedRelayOrigin(value: string): boolean {
  try {
    ...
    const privateIpv4 = isIP(url.hostname) === 4 &&
      (/^10\./.test(url.hostname) || /^192\.168\./.test(url.hostname) ||
        /^172\.(1[6-9]|2\d|3[01])\./.test(url.hostname))
    const privateIpv6 = isIP(url.hostname) === 6 &&
      (url.hostname.toLowerCase().startsWith('[fc') || url.hostname.toLowerCase().startsWith('[fd'))
    return url.origin === value &&
      (url.protocol === 'https:' || (url.protocol === 'http:' && (loopback || privateIpv4 || privateIpv6)))
  } catch {
    return false
  }
}
