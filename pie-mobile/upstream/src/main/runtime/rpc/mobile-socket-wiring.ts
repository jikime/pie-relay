...
export class MobileSocketWiring {
  ...
  // Relay assignment can move to another cell, and the host gateway replaces
  // its transport when that happens. Forget the retired transport so device
  // revocation does not retain/revisit every transport created by reconnects.
  // The transport is stopped by its owner before or immediately after detach.
  detachTransport(transport: MobileSocketTransport): void {
