...
const PairingConnectionModeSchema = z.enum(['automatic', 'local-only', 'relay-only'])
...
function isAllowedRelayOrigin(value: string): boolean {
  ...
  try {
    ...
    const privateIpv6 = parsed.hostname.toLowerCase().startsWith('[fc') || parsed.hostname.toLowerCase().startsWith('[fd')
    return value === parsed.origin &&
      (parsed.protocol === 'https:' || (parsed.protocol === 'http:' && (loopback || privateIpv4 || privateIpv6)))
  } catch {
    return false
  }
}
...
export function createPairingOfferSchema(now: () => number = () => Date.now()) {
  const relaySchema = z.object({
    ...
    directorUrl: z
      .string()
      .min(1)
      .refine(isAllowedRelayOrigin, 'Expected HTTPS or loopback HTTP origin'),
    cellUrl: z.string().min(1).refine(isAllowedRelayOrigin, 'Expected HTTPS or loopback HTTP origin'),
  ...
  return z
    .object({
      ...
      connectionMode: PairingConnectionModeSchema.optional(),
    ...
    .superRefine((offer, ctx) => {
      ...
      if (offer.connectionMode === 'relay-only' && !offer.relay) {
        ctx.addIssue({
          code: 'custom',
          path: ['connectionMode'],
          message: 'Relay-only pairing requires Relay metadata'
        })
      }
