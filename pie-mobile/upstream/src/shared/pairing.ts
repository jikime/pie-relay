...
export function encodePairingOffer(offer: PairingOffer): string {
  ...
  return `pierelay://pair?code=${base64url}`
}

export function decodePairingOffer(url: string): PairingOffer {
  const code = extractPairingCodeFromUrl(url)
  if (!code) {
    throw new Error('Invalid pairing URL: must use pierelay://pair and include a pairing code')
  }
...
function extractPairingCodeFromUrl(url: string): string | null {
  ...
  if ((parsed.protocol !== 'pierelay:' && parsed.protocol !== 'orca:') || parsed.hostname !== 'pair') {
    return null
  }
...
// Why: accept either a current/legacy pairing URL or the bare base64
...
export function parsePairingCode(input: string): PairingOffer | null {
  ...
  try {
    if (/^(pierelay|orca):\/\//i.test(trimmed)) {
