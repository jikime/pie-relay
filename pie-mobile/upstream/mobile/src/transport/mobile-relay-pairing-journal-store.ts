...
export async function saveMobileRelayPairingJournal(
  ...
  const mutation = journalMutation.then(async () => {
    ...
    if (
      ...
      existing.relay.inviteExpiresAt > Date.now()
    ) {
    ...
    // Once an authorized attempt's invite has expired, an explicit new scan may
    // also replace it. If its pending resume token had actually been committed,
    // replacing the journal only abandons a mobile-held token that nobody else
    // knows; the fresh QR provisions a new device credential without exposing it.
...
function requireNativeSecretStore(): void {
  if (Platform.OS === 'web') {
    throw new Error('Pie Relay pairing requires a native secret store')
  }
}
