...
async function runPairing(
  ...
  if (offer.connectionMode !== 'relay-only') {
  ...
  if (journal && offer.connectionMode !== 'local-only') {
  ...
  if (candidates.length === 0) {
    throw new Error('pairing offer has no usable transport')
  }
  ...
  await dependencies.saveHost(relayHost(journal, endpoints.relay, offer.connectionMode))
  await dependencies.clearJournal(journal.metadata.journalId)
  return { hostId }
}
