...
describe('mobile relay pairing journal store', () => {
  ...
  it('blocks replacement when the old authorization update wins the mutation race', async () => {
    ...
    const authorizationUpdate = updateMobileRelayPairingJournal(
      ...
      (metadata) => ({
        ...metadata,
        relay: { ...metadata.relay, inviteExpiresAt: Date.now() + 60_000 },
        winner: 'direct',
        authorizationMode: 'authenticated-direct'
      })
    )
  ...
  it.each([
    ...
    const journal = {
      ...
      metadata: {
        ...created.metadata,
        relay: { ...created.metadata.relay, inviteExpiresAt: Date.now() + 60_000 },
        ...authorization
      }
    }
  ...
  it('allows an explicit new scan to replace an authorized journal after its invite expires', async () => {
    const created = createMobileRelayPairingJournal({
      offer: offer as PairingOffer & { relay: NonNullable<PairingOffer['relay']> },
      hostId: 'host-1',
      hostName: 'Blue Whale',
    ...
    const expired = {
      ...
      metadata: {
        ...created.metadata,
        relay: { ...created.metadata.relay, inviteExpiresAt: Date.now() - 1 },
        winner: 'relay' as const,
        authorizationMode: 'relay-basis' as const
      }
    }
    await saveMobileRelayPairingJournal(expired)

    const replacement = createMobileRelayPairingJournal({
      offer: {
        ...offer,
        relay: { ...offer.relay, inviteExpiresAt: Date.now() + 300_000 }
      } as PairingOffer & { relay: NonNullable<PairingOffer['relay']> },
      hostId: 'host-2',
      hostName: 'Red Panda',
      randomBytes: (length) => new Uint8Array(length).fill(12)
    })
