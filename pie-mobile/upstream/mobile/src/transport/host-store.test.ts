...
describe('host-store list mutations', () => {
  ...
  it('migrates an older relay-only overlay without reopening its base LAN endpoint', async () => {
    const relay = {
      v: 1 as const,
      directorUrl: 'https://relay.onorca.dev',
      cellUrl: 'https://relay-c1.onorca.dev',
    ...
    storedOverlayRaw = JSON.stringify([
      {
        v: 2,
        hostId: HOST_ONE.id,
        endpoints: [
  ...
  it('persists relay-only policy in the compatibility overlay namespace', async () => {
    const relay = {
      v: 1 as const,
      directorUrl: 'https://relay.onorca.dev',
      cellUrl: 'https://relay-c1.onorca.dev',
