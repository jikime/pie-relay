...
function journal(
  ...
  connectionMode?: PairingOffer['connectionMode']
) {
  const value = createMobileRelayPairingJournal({
    offer: { ...offer, connectionMode } as PairingOffer & { relay: NonNullable<PairingOffer['relay']> },
    hostId: 'host-1',
    hostName: 'Blue Whale',
    now,
...
describe('mobile relay pairing recovery', () => {
  ...
  it('recovers relay-only pairing without publishing a LAN endpoint', async () => {
    const saved = journal('relay-basis', 'relay-only')
    ...
    expect(deps.saveHost).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        endpoints: [expect.objectContaining({ kind: 'relay' })]
      })
    )
    expect(deps.saveHost.mock.calls[0]![0].endpoints).not.toContainEqual(
