...
async function doLoadHosts(): Promise<HostProfile[]> {
  ...
  for (const stored of storedHosts) {
    ...
    // Why: relay-only is a durable security/routing policy, not a transient
    // QR-screen preference. Older v2 overlays did not persist the mode; an
    // overlay containing only relay endpoints is an unambiguous migration
    // signal and must never reopen the legacy LAN endpoint after restart.
    const connectionMode =
      overlay?.connectionMode ??
      (overlay?.endpoints.every(({ kind }) => kind === 'relay') ? 'relay-only' : undefined)
    out.push({
      ...stored,
      deviceToken: token,
      ...(overlay
...
async function persistHost(host: HostProfile, requireExisting: boolean): Promise<void> {
  ...
  if (validated.endpoints) {
    await saveMobileRelayHostOverlay({
      ...
      connectionMode: validated.connectionMode
    })
  }
}
