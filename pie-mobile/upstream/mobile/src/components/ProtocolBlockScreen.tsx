...
const RELEASES_URL = 'https://cookai.dev'
const IOS_APP_STORE_URL = 'https://cookai.dev'
...
export function ProtocolBlockScreen({ verdict }: Props) {
  ...
  const primaryAction = isMobileTooOld
    ? mobileUpdateTarget.url && mobileUpdateTarget.label
      ...
      : null
    : { label: 'Open cookai.dev', url: RELEASES_URL }

  const title = isMobileTooOld ? 'Update Pie Relay Mobile' : 'Update Pie Relay on your computer'
  const body = isMobileTooOld
    ? `This desktop needs a newer Pie Relay Mobile app. Update Pie Relay Mobile from ${mobileUpdateTarget.storeName}, then try this host again.`
    : 'This paired desktop app is too old for your current Pie Relay Mobile app. Update Pie Relay on your computer, then try this host again.'
  const recoveryNote =
