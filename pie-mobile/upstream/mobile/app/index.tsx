...
import { PieRelayLogo } from '../src/components/PieRelayLogo'
...
function fallbackConnectionPath(host: HostProfile): 'lan' | 'tailscale' | 'relay' {
  if (
    host.connectionMode === 'relay-only' ||
    (host.endpoints?.length && host.endpoints.every(({ kind }) => kind === 'relay'))
  ) {
    return 'relay'
  }
  try {
    const hostname = new URL(host.endpoint).hostname
...
function formatDuration(ms: number | null | undefined): string {
  const normalizedMs = Number.isFinite(ms) ? Math.max(0, ms ?? 0) : 0
      ...
      <View style={styles.topBar}>
        <View style={styles.brandLockup}>
          <View style={styles.logoMark}>
            <PieRelayLogo size={20} />
          </View>
          <Text style={styles.brandName}>Pie Relay</Text>
        </View>
        <Pressable
        ...
        >
          <View style={styles.emptyHero}>
            ...
            <Text style={styles.emptyBody}>
              Pair with Pie Relay on your computer to check on your agents, jump into any terminal, and
                ...
                <View style={styles.statsRow}>
                  <View style={styles.statCard}>
                    <Text style={styles.statValue}>
                      {(stats.totalAgentsSpawned ?? 0).toLocaleString()}
                    </Text>
                  ...
                  <View style={styles.statCard}>
                    <Text style={styles.statValue}>{(stats.totalPRsCreated ?? 0).toLocaleString()}</Text>
          ...
          renderItem={({ item }) => {
            ...
            return (
              <MobileHostCard
                ...
                state={state}
                verdict={verdict}
                path={hostPaths[item.id] ?? fallbackConnectionPath(item)}
                worktreeCounts={
